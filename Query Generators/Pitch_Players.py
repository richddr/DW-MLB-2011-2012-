...
file = open("../CSV/pitching2011.txt")
fileoutput = open('../Queries/Pitching_Queries2011.txt', 'w')
i = 428
year = "2011";
...
while 1:
    ...
    if not line:
        ...
        flag = False 
        line_s = line.split(',')
        insert_l= "insert into PITCHEO_JUGADOR_EQUIPO values( "+ str(i) +", "+ team_rep +", " + year

        for x in range(1, len(line_s)):
            if(len(line_s[x]) == 0):
                if x == 1:
                    line_s[x] = "P"
                else:
